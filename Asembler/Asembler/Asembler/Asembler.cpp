...
void popuni_instrukcije(std::unordered_map<std::string, Instrukcija*>& inst) {
	...
	inst["push"] = new JedanOpInst<0x40>();
	inst["pop"] = new JedanOpInst<0x41>();
	inst["rand"] = new JedanOpInst<0x42>();
	inst["ret"] = new BezadresneInst<0x60>();
	inst["sret"] = new BezadresneInst<0x61>();
	inst["inte"] = new BezadresneInst<0x62>();
	inst["intd"] = new BezadresneInst<0x63>();
	inst["srand"] = new BezadresneInst<0x64>();
	inst["inc"] = new JedanOpInst<0x8A>();
	inst["dec"] = new JedanOpInst<0x8B>();
	inst["cl"] = new JedanOpInst<0x8C>();
	inst["not"] = new JedanOpInst<0x8D>();
	...
	inst["li"] = new AritmetickoNeposredne<0xA4>();
	inst["mv"] = new AritmetickoRegistarske<0xA5>();
